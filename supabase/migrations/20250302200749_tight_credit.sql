...
DROP FUNCTION IF EXISTS check_content_moderation(text, text, text, text);
DROP FUNCTION IF EXISTS check_content_moderation(text, text, text);

-- Create content moderation function with correct parameter order
...
BEGIN
  ...
  v_content_id := gen_random_uuid()::text;
