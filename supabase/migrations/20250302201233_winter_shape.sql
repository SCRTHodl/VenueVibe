...
DROP FUNCTION IF EXISTS handle_story_interaction CASCADE;

-- Create optimized record_story_view function with reduced recursion
...
AS $$
BEGIN
  -- Update story view count and viewed_by array atomically
  UPDATE stories s
  SET 
  ...
  -- Update analytics with minimal recursion
...
-- Create optimized story interaction function
...
CREATE INDEX IF NOT EXISTS idx_story_likes_unique 
...
-- Create cron job to run cleanup periodically
