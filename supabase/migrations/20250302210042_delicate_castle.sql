...
DECLARE
  ...
  v_viewed_by uuid[];
BEGIN
  -- Update story and get viewed_by array atomically
  UPDATE stories
  SET 
  ...
  WHERE id = p_story_id
  RETURNING viewed_by INTO v_viewed_by;

  -- Calculate unique viewers
  SELECT COUNT(DISTINCT x) INTO v_unique_viewers
  FROM unnest(v_viewed_by) x;
